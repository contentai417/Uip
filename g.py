...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        f"👉 [Click Here to Join](https://t.me/+2OeR_LqYChMyZDQ9)\n"
        "After joining the channel, click 'I Have Joined' to proceed."
    )
    
    keyboard = [
        [InlineKeyboardButton("📢 Join Channel", url=f"https://t.me/+2OeR_LqYChMyZDQ9")],
...
async def genkey(update: Update, context: CallbackContext):
    if update.callback_query:  # Check if called from a button click
        ...
        if user_id in ADMIN_IDS:
            ...
            await context.bot.send_message(user_id, "ONLY OWNER CAN USE💀OWNER @YTABHIA2Z")
        ...
        if user_id in ADMIN_IDS:
            ...
            response = "ONLY OWNER CAN USE💀OWNER @YTABHIA2Z"
...
async def handle_already_approved(update: Update, context: CallbackContext):
    ...
    if user_id in users:  # Check if the user has already redeemed a key
        ...
        await context.bot.send_message(user_id, "❌ *You have not redeemed a key yet!* Please redeem a key first. You can buy it from @YTABHIA2Z.")
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        ...
        await show_all_buttons(chat_id, context)  # Pass contex
...
BOT_TOKEN = '8157026267:AAF0rQ6pQcOOiKFfIfK8oOE0TRlhR9GVbB4'
CHANNEL_USERNAME = 'FUCK'
ADMIN_IDS = ['5388393253']  # Admin user IDs
